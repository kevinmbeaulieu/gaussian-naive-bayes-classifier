...
public class GaussianNaiveBayesExample extends Applet {
	...
	public GaussianNaiveBayesExample() {
		...
		categoryColors.put("red", Color.red);
		categoryColors.put("green", Color.green);
		categoryColors.put("blue", Color.blue);
		categoryColors.put("yellow", Color.yellow);
		
		training = new ArrayList<>();
		
		/*Random r = new Random();
		for (int i = 0; i < 100; i++) {
			training.add(new ClassifiedVector("red", new Vector(200 + r.nextInt() % 50, 200 + r.nextInt() % 100)));
		}
		for (int i = 0; i < 100; i++) {
			training.add(new ClassifiedVector("green", new Vector(400 + r.nextInt() % 50, 400 + r.nextInt() % 200)));
		}
		for (int i = 0; i < 100; i++) {
			training.add(new ClassifiedVector("blue", new Vector(600 + r.nextInt() % 50, 280 + r.nextInt() % 250)));
		}*/
		...
		for (String str : inputSplit) {
			...
			if (category.equals("red") && r.nextBoolean()) category = "green";
	...
	public void paint(Graphics g) {
		...
		int numCategories = categoryColors.size();
		for (ClassifiedVector point : predicted) {
			float[] rgb = new float[3];
			for (Map.Entry<String, Double> entry : point.classification.getCategoryProbabilities().entrySet()) {
				Color categoryColor = categoryColors.getOrDefault(entry.getKey(), Color.black);
				float[] categoryRgb = categoryColor.getRGBColorComponents(null);
				for (int j = 0; j < 3; j++) {
					rgb[j] += (0.4 + categoryRgb[j] * 0.6) * entry.getValue();
				}
			}
			outputGraphics.setColor(new Color(rgb[0], rgb[1], rgb[2]));
		...
		for (ClassifiedVector point : training) {
			Color categoryColor = categoryColors.getOrDefault(point.classification.getCategory(), Color.black);
			outputGraphics.setColor(categoryColor);
